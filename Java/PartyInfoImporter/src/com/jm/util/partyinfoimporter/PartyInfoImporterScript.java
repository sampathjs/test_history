...
public class PartyInfoImporterScript implements IScript
{
	...
	private void removeAllExceptRetainedParties(Table outtable,
	...
	private void updatePartyInfoField(Table outtable, String partyName,
			String partyInfoField, String partyInfoFieldValue, Set<Integer> toRetain) throws OException {
    	outtable.sortCol("short_name");
    	int rowPartyTable = outtable.findString("short_name", partyName, SEARCH_ENUM.FIRST_IN_GROUP);
    	if (rowPartyTable < 1) {
    		throw new OException ("Could not update party '" + partyName + "', party info '"
    				+ partyInfoField + "' with value '" + partyInfoFieldValue + "'"
	...
	private int getPartyInfoId(String partyInfoField, int partyClass, int intExt,
			String partyName) throws OException {
		String sql = "\nSELECT type_id"
				+ "\nFROM party_info_types"
				+ "\nWHERE type_name = '" + partyInfoField + "'"
				+ "  \nAND party_class = " + partyClass
				+ "  \nAND int_ext = " + intExt
				;
		Table retTable = null;
		try {
    	    ...
    	    if (ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.jvsValue() ||
    	    	retTable.getNumRows() == 0) {
    	    	throw new RuntimeException("Error retrieving ID of party info field '"
    	    			+ partyInfoField + "' for party '" + partyName + "'. Does it exist?");
    	    }
    ...
    private void updatePartyInfoFields(Table outtable, Set<Integer> toRetain) throws OException {
    	...
    	updatePartyInfoField (outtable, "JM CHEMICALS INDIA (TALOJA) PVT LTD - BU", "Customer Code GBP", "33321", toRetain);
    	updatePartyInfoField (outtable, "JM CHEMICALS INDIA (TALOJA) PVT LTD - BU", "Customer Code USD", "33312", toRetain);
    	...
    	updatePartyInfoField (outtable, "JOHNSON MATTHEY (ZHANGJIAGANG) PRECIOUS METALS TECHNOLOGY CO LTD - BU", "Customer Code GBP", "44661", toRetain);
    	updatePartyInfoField (outtable, "JOHNSON MATTHEY AGT MEXICO - BU", "Customer Code GBP", "44708", toRetain);
    	updatePartyInfoField (outtable, "JOHNSON MATTHEY APPLIED MATERIALS TECHNOLOGIES - BU", "Customer Code GBP", "44687", toRetain);
    	updatePartyInfoField (outtable, "JOHNSON MATTHEY COLOUR TECHNOLOGIES KOREA - BU", "Customer Code GBP", "44644", toRetain);
    	updatePartyInfoField (outtable, "JOHNSON MATTHEY PIEZO PRODUCTS GMBH - BU", "Customer Code GBP", "44601", toRetain);
    	updatePartyInfoField (outtable, "JOHNSON MATTHEY SHANGHAI CATALYSTS - BU", "Customer Code GBP", "44610", toRetain);
    	updatePartyInfoField (outtable, "JOHNSON MATTHEY SHANGHAI CATALYSTS - BU", "Customer Code USD", "35191", toRetain);
    	...
    	updatePartyInfoField (outtable, "ROYAL BANK OF CANADA, CAPITAL MARKETS - BU", "Customer Code GBP", "45874", toRetain);
    	updatePartyInfoField (outtable, "ROYAL BANK OF CANADA, CAPITAL MARKETS - BU", "Customer Code USD", "45866", toRetain);
    	...
    	updatePartyInfoField (outtable, "JM TREASURY - BU", "Customer Code ZAR", "JM TREASURY - ZAR", toRetain);
    	...
    	updatePartyInfoField (outtable, "SHELL CATALYSTS & TECHNOLOGIES LTD - BU", "Customer Code GBP", "33523", toRetain);
    	...
    	updatePartyInfoField (outtable, "EVONIK PERFORMANCE MATERIALS GMBH - BU", "Customer Code EUR", "44783", toRetain);
    	...
    	updatePartyInfoField (outtable, "EXXONMOBIL CATALYST SERVICES INC - BU UK", "Customer Code USD", "17161", toRetain);
    	...
    	updatePartyInfoField (outtable, "GM EUROMETALS, INC CORPORATION TRUST CENTER - BU", "Customer Code EUR", "30621", toRetain);
    	updatePartyInfoField (outtable, "GM EUROMETALS, INC CORPORATION TRUST CENTER - BU", "Customer Code GBP", "30605", toRetain);
    	updatePartyInfoField (outtable, "GM EUROMETALS, INC CORPORATION TRUST CENTER - BU", "Customer Code USD", "30613", toRetain);
