...
public class MDTLoaderLME_AM implements IScript {

	public void execute(IContainerContext context) throws OException
	{
		setUpLog();
		
		PluginLog.debug("START MDTLoaderLME_AM param");
		
		boolean blnAllPricesImported = PriceImportReporting.allPricesImported("LME AM","", OCalendar.today());
		
		if(blnAllPricesImported == false){
			
			PluginLog.info("Importing prices for LME AM on " + OCalendar.formatJd(OCalendar.today()));

		    String loadGroup1;
			
		    loadGroup1 = "RefSource LME AM";
		...
		if(blnAllPricesImported == true){
			...
			PluginLog.debug("END MDTLoaderLME_AM param");
