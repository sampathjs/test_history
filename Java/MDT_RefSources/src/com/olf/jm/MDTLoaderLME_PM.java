...
public class MDTLoaderLME_PM implements IScript {
	...
	public void execute(IContainerContext context) throws OException
	{
		...
		PluginLog.debug("START MDTLoaderLME_PM param");
		
		boolean blnAllPricesImported = PriceImportReporting.allPricesImported("LME PM","", OCalendar.today());
		
		if(blnAllPricesImported == false){
			
			PluginLog.info("Importing prices for LME PM on " + OCalendar.formatJd(OCalendar.today()));
		...
		if(blnAllPricesImported == true){
			...
			PluginLog.debug("END MDTLoaderLME_PM param");
