...
public class PurgeHistoryData implements IScript
{
	...
	public void execute(IContainerContext context) throws OException
	{
		...
		try
		{
			...
			String reportFileName = Util.reportGetDirForToday().replace("/", "\\") + "\\Purge_Report_" + 
							  OCalendar.formatDateInt(OCalendar.today(), DATE_FORMAT.DATE_FORMAT_DMLY_NOSLASH) + "_" + Util.timeGetServerTime() + ".csv";
	...
	*/
	 ...
	 * @throws OException
	 */
	private String PurgeOLUsingFunctions(Table purgeNamesToPurge,
			Table purgeToTableMap, boolean checkCategory, String purgeCategory) throws OException {
		...
		for ( int row = 1; row <= numPurgeNames; row++)
		{
			...
			if ( checkCategory && !purgeCategory.equals(category) ) {
				PurgeUtil.printWithDateTime("Skipping purge: " + purgeName + ", category: " + category 
						+ ". Does not match specified category: " + purgeCategory);
				continue;
			}
			...
			if ("STLDOC_HIST".equalsIgnoreCase(purgeName)) 
			{
				...
				try {
					...
					if ( ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt() ) {
						messageText += "!!!ERROR!!! Failed to purge OL archive type: " + purgeName + ": error code"
								+ ret + "\n";
					}
				} catch (Exception e) {
					messageText += "!!!ERROR!!! Failed to purge OL archive type: " + purgeName + ": "
							+ e.getMessage() + "\n";
				} finally {
					start.destroy();
					end.destroy();
			...
			else if ("TPM_ORPHAN_WORKFLOWS".equalsIgnoreCase(purgeName) ||
					"TPM_DISCONNECTED_VARIABLES".equalsIgnoreCase(purgeName)) 
			{
	...
	*/
	private void GetAfterStatsForAffectedTables(String purgeName, Table purgeToTablesMap) throws OException {
		...
		for ( int tableRow = firstRow; tableRow <= lastRow; tableRow++) {
			...
			if ( numberRowsPurged < 0 ) // guess this could happen if 
			   numberRowsPurged = 0;
