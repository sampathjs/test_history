...
public class TransferDestinationTypeValidator extends FieldValidatorBase {
	...
	/**
	 * Validates the Transfer destination type of request
	 * with transfer types configured in COnnex Mapings
	 * @param value
	 * @return boolean 
	 */
	private boolean validTransferDestType(String value) {
		...
		try {
			...
			String errorMessage = "Error validating Metal Transfer Type "
					+ e.getMessage() + "\n";
	...
	/**
	 * loads the Transfer destination Type configured in the 
	 * COnnex Mappings
	 * @return Table  
	 */
	private HashSet<String> loadConnexDataMapping() throws Exception {
		...
		try {
			String sql = "SELECT"
					+ " CASE WHEN ma.name IS NOT NULL"
					+ " THEN ma.name"
					+ " ELSE ts.name"
					+ " END as alias"
					+ " FROM oc_ref_map_data md"
					+ " JOIN oc_ext_ref_data  rd ON (md.ext_ref_id = rd.ext_ref_id )"
					+ " JOIN TRANS_STATUS  ts on (md.int_ref_id = ts.trans_status_id)"
					+ " LEFT JOIN master_alias_table mat ON ( mat.name = 'TRANS_STATUS')"
					+ " LEFT JOIN master_aliases ma ON (ma.id_number = mat.id_number AND ts.trans_status_id = ma.ref_id)"
