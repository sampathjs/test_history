 ...
 */
public class FromSegmentValidator extends FieldValidatorBase {
	 ...
	 */
	public FromSegmentValidator(final ISapPartyData currentPartyData) {
	...
	public final String getFieldName() {
		return EnumSapTransferRequest.FROM_SEGMENT.getColumnName();
	}
	...
	public final void validate(final String value) throws ValidatorException {
		...
		if (value.equals(partyData.getFromAccount().getSegment())) {
			if (partyData.getFromAccount().getAccountBusinessUnit() == null 
					||  partyData.getFromAccount().getAccountBusinessUnit().length() == 0) {
	...
	protected final String getFieldErrorDesc() {
		return "From segment failed validation. Invalid from segment / from company code / from account number";
	}
