...
import com.olf.jm.coverage.messageValidator.fieldValidator.CoverageAccountValidator;
...
public class CoverageValidator extends ValidatorBase  {
	 ...
	 */
	@Override
	protected final void initValidators(
		...
		IFieldValidator quoationRefValidator = new QuotationRefValidator(
				context, currentTemplateData);
		validators.add(quoationRefValidator);

		IFieldValidator instIdValidator = new InstrumentIdValidator(context,
				currentTemplateData);
		validators.add(instIdValidator);

		IFieldValidator timeCodeValidator = new TimeCodeValidator(context,
				currentTemplateData);
		...
		IFieldValidator currencyValidator = new CoverageCurrencyValidator(
				context);
		...
		IFieldValidator valueDataValidator = new ValueDate(currentTemplateData);

		validators.add(valueDataValidator);

		IFieldValidator metalElementValidator = new CoverageMetalElementValidator(
				context);
		validators.add(metalElementValidator);

		IFieldValidator buValidator = new CoverageBusinessUnitCodeValidator(
				currentSapPartyData, context);
		validators.add(buValidator);
		
		IFieldValidator accountValidator = new CoverageAccountValidator(
				currentSapPartyData);
		...
		IFieldValidator tradingDeskIdValidator = new TradingDeskIdValidator(context,
				currentSapPartyData);
	 ...
	 */
	protected void initTwoFieldValidators() {
		// No two Field validator for coverage trade
