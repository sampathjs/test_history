...
Script Description:              This script allows to import of Batches from user table.
								 Tables used are USER_migr_us_batches and USER_migr_op_batchdata_fields.
								 USER_migr_us_batches - this table contains the data coming from external system.
...
public class Migr_Batch_forUS extends AbstractGenericScript {
	...
	public Table execute(Context context, ConstTable table) {
		...
		//Get the data from USER_migr_us_batches
		String SqlString = "select * from USER_migr_us_batches where row_type='2'";
		try
		{
		...
		{
			PluginLog.error("\n Check sqlCommand: " + SqlString+ "\n Unable to load USER_migr_us_batches table");
		...
		String smap= sb.toString() + " from USER_migr_us_batches where row_type='2';";
		...
		{
			PluginLog.error("\n Check sqlCommand: " + smap +"\n Unable to load Batch and container mapped data onto the table USER_migr_us_batches");
			PluginLog.error("\n Error: " + Error.getLocalizedMessage());
		}
		...
		String smapmeasure= sbmeasure.toString() + " from USER_migr_us_batches where row_type='2';";
		...
		{
			PluginLog.error("\n Check sqlCommand: " + smapmeasure+"\n Unable to Measures mapped data onto the table USER_migr_us_batches");
			PluginLog.error("\n Error: " + Error.getLocalizedMessage());
		}
	...
	String session = "starting";
		...
		for(idata=0; idata<imigrrownum; idata++){
           ...
           catch (Exception Error) 
   		{
            ...
            if (simporttype.equalsIgnoreCase(batmeacont)){
            String sel= sb.toString() + " from USER_migr_us_batches where row_type='2' AND sadbtn='"+ batnum +"'";
            try
       		{
           ...
           try{
               //update the status of the row in the USER table to update status appropriately
                 com.olf.openjvs.Table USER_migr_us_batches;
              	USER_migr_us_batches= com.olf.openjvs.Table.tableNew();
              	// Set the name attribute of the table. This should be the name of the database table
              	USER_migr_us_batches.setTableName("USER_migr_us_batches");
               DBUserTable.load(USER_migr_us_batches);
               USER_migr_us_batches.sortCol("row_id");
              	int row= USER_migr_us_batches.findInt("row_id", controwid, SEARCH_ENUM.LAST_IN_GROUP);
              	USER_migr_us_batches.setString("status_flag", row, "BOOKED");
               	USER_migr_us_batches.setString("target_status_flag",row , "BOOKED");
               	USER_migr_us_batches.setInt("row_type",row , 6);
               	USER_migr_us_batches.setString("status_msg", row, errormsg);
               	USER_migr_us_batches.group("row_id");
      		
               	int iRC=DBUserTable.update(USER_migr_us_batches);
            ...
            try{
                ...
                DBUserTable.load(USER_migr_us_batches);
                USER_migr_us_batches.sortCol("row_id");
               	int row= USER_migr_us_batches.findInt("row_id", irowid, SEARCH_ENUM.LAST_IN_GROUP);
               	USER_migr_us_batches.setInt("batch_id", row, batchId1  );
               	USER_migr_us_batches.setInt("delivery_id", row, nomId1  );
               	USER_migr_us_batches.setString("status_flag", row, "BOOKED");
               	USER_migr_us_batches.setString("target_status_flag",row , "BOOKED");
               	USER_migr_us_batches.setInt("row_type",row , 6);
               	USER_migr_us_batches.setString("status_msg", row, "Successfully Booked");
               	USER_migr_us_batches.group("row_id");
	 ...
	 try{
         //update the status of the row in the USER table to update status appropriately
           com.olf.openjvs.Table USER_migr_us_batches;
        	...
        	DBUserTable.load(USER_migr_us_batches);
        	USER_migr_us_batches.sortCol("row_id");
        	USER_migr_us_batches.viewTable();        	
    ...
    public void errorupdate(int rownum, String msg) throws OException{
		 ...
		 try
        	...
        	USER_migr_us_batches.setTableName("USER_migr_us_batches");
         	DBUserTable.load(USER_migr_us_batches);
         	USER_migr_us_batches.sortCol("row_id");
        	int row=USER_migr_us_batches.findInt("row_id", rownum, SEARCH_ENUM.LAST_IN_GROUP);
        	
        	USER_migr_us_batches.setString("status_msg", row, msg);
        	USER_migr_us_batches.setInt("error_flag", row , 1);
