...
public class Migr_RefData_Contacts_Updates implements IScript
{
    ...
    public void execute(IContainerContext context) throws OException
    {
		...
		//Get all the required data from USER table to be imported
	  	boolean setFunctionalGroups = false;
	  	boolean setPersonnelInfo = false;
		try 
		{
			DBaseTable.execISql(tPersonnel, sPersonnel);
			setFunctionalGroups = tPersonnelData.unsortedFindString("endur_field_name", "func_group_id", SEARCH_CASE_ENUM.CASE_INSENSITIVE) > 0 ? true : false;
			setPersonnelInfo= tPersonnelData.unsortedFindString("endur_field_name", "JM Base Price Template", SEARCH_CASE_ENUM.CASE_INSENSITIVE) > 0 ? true : false;
	    ...
	    for(int iPersonnel=1; iPersonnel<=rowCountPersonnel; iPersonnel++){
	    	...
	    	String ContactName= tPersonnel.getString("short_name", iPersonnel);
	  		String queryName= "Select id_number from personnel where name= '"+ ContactName +"';";
			...
			catch (OException Error)
			{
	  		...
	  		int cid= name.getInt(1, 1);
	    	
	    	Table personal;

			personal = Table.tableNew("Personnel");

	    personal=Ref.retrievePersonnel(cid);//Retrieve Personnel Table
	      
	    personal.setInt("status", 1, 1);
	    
	    // Only clear out date if functional groups are active
	  	if(setFunctionalGroups) {	  	
	  		personal.getTable("personnel_functional_group", 1).clearRows();
	  	}
	    if(setPersonnelInfo) {
	    	personal.getTable("personnel_info", 1).clearRows();
	    }
	    			...
	    			|| EndurPersonnelfieldLE.equalsIgnoreCase("short_alias_name")|| EndurPersonnelfieldLE.equalsIgnoreCase("phone")|| EndurPersonnelfieldLE.equalsIgnoreCase("name"))
	    		{
	    			...
	    			personal.setInt(EndurPersonnelfieldLE, 1, Ref.getValue(SHM_USR_TABLES_ENUM.STATES_TABLE,retPersonnelfieldLE));
	    			      ...
	    			      for (int xlength=0; xlength<resultf.length; xlength++)
	    			      {
	    			    	  ...
	    			    	  int funcGroupId = Ref.getValue(SHM_USR_TABLES_ENUM.FUNCTIONAL_GROUP_TABLE, splitStringf);
	    			    	  
	    			    	  if(funcGroupId < 1) {
	    			    		   String errorMessage = "Error looking up functional group [" + splitStringf + "]";
	    			    			PluginLog.error(errorMessage);
	    			    			throw new OException(errorMessage); 
	    			    	  }
	    			    	  personal.getTable("personnel_functional_group", 1).setInt(EndurPersonnelfieldLE, xlength+1, funcGroupId);
	    			      }
      	    		}
	    ...
	    personal.setColValInt("save_personnel_info_flag", 1);
	    		...
	    		catch (OException Error)
	    		{
	         	...
	         	user_migr_p_contacts.setString("status_flag", row, "UPDATED");
	         	user_migr_p_contacts.setString("target_status_flag",row , "UPDATED");
	         	user_migr_p_contacts.setInt("row_type",row , 7);
	         	user_migr_p_contacts.setString("status_msg", row, "Successfully UPDATED");
