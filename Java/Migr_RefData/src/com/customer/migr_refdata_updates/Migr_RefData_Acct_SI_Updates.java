 ...
 * 2017-04-11	V1.1	jwaechter	- bugfixings
 *                                  - update
 *                                  - skipping
 *                                  
 */
...
public class Migr_RefData_Acct_SI_Updates implements IScript
{
	...
	public void execute(IContainerContext context) throws OException
	{
		...
		first:
			for(int iAcct=1; iAcct<=rowCountAcct;iAcct++)//
			{
				...
				int Acctid = Ref.getValue(SHM_USR_TABLES_ENUM.ACCOUNT_TABLE, AcctName);
				
				if (Acctid < 1) {
					String errormsg = "Account '" + AcctName + "' does not exist. Skipping update.";
					Logging.error(errormsg+ " \n Couldn't process row no. " + iAcct + " for row_id " + rowid);
					errorupdate(rowid, errormsg, select, true);
					continue first;
				}
...
//				acct.getTable("account_info", 1).clearRows();
				...
				for(int irefdata=1;irefdata<=rowCountRefdata; irefdata++)
				{
					...
					if((retfieldAcc== null||(retfieldAcc.trim().equalsIgnoreCase("")))&& reqflag.equalsIgnoreCase("Yes"))
					{
						...
						Logging.error(errormsg+ " \n Couldn't process row no. " + iAcct + " for row_id " + rowid);
						errorupdate(rowid, errormsg, select, false);
						continue first;	
					}
					...
					else
					{
						try{
							...
							else if (retfieldAcc != null && !retfieldAcc.trim().equals(""))
							{
								...
								acct.getTable("account_info", 1).sortCol("info_type_id");
								int iInforows = acct.getTable("account_info", 1).findInt("info_type_id", itypeIdActInfo, SEARCH_ENUM.FIRST_IN_GROUP);
								if (iInforows < 1) {
									iInforows = acct.getTable("account_info", 1).addRow();
								}
				...
				int settid = Ref.getValue(SHM_USR_TABLES_ENUM.SETTLE_INSTRUCTIONS_TABLE, AcctName);
				if (settid < 1) {
					String errormsg = "SI for account'" + AcctName + "' does not exist. Skipping update.";
					Logging.error(errormsg+ " \n Couldn't process row no. " + iAcct + " for row_id " + rowid);
					errorupdate(rowid, errormsg, select, true);
					continue first;
				}
...
//				settle.getTable("stl_ins", 1).clearRows();
	...
	private void errorupdate(int rownum, String msg, String select, boolean skip) throws OException{
		...
		try
		{
			...
			if (skip) {
				user_migr_p_acct_si.setInt("row_type", row , 9);								
			}
