          ...
          //Get mapping information from mapping table in USer Tables. This maps input table to Endur field names
    		...
    		for (iPGLoop = 1; iPGLoop <=rowCountpg ; iPGLoop++) {
              ...
              //Set group names for BU and LE
                ...
                try{
                RefBase.importPartyTable(table, 1); // complete_flag=1 and it is unused
                }
                catch(OException Error){
                ...
                try
                	...
                	user_migr.setString("status_flag", row, "BOOKED");
                	user_migr.setString("target_status_flag", row , "BOOKED");
                	user_migr.setInt("row_type", row , 6);
                	user_migr.setString("status_msg", row, "Successfully Booked");
