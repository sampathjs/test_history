/********************************************************************************
 * Project: WGS
 * Script Name: AutoRollOtherDates
 * Script Type: main
 * Status: complete
 *
 * Revision History:
...
/**
 * This script rolls all dates, except trading date.
 * @author jbonetzk
 * @version 1.1
 *
 * Dependencies:
 ...
 */
public class AutoRollOtherDates implements IScript
{
    ...
    {    
        ...
        catch (OException oe)
        {
            ...
            AlertBroker.sendAlert ("EOD-ARO-004", strMessage);
        }finally{
        	Logging.close();
        }
    ...
    void initPluginLog () throws OException
    {
        ...
        catch (Exception ex)
        {
            ...
            AlertBroker.sendAlert ("EOD-ARO-001", strMessage);
            Util.exitFail ();
        }
    }
    
    void rollOtherDates() throws OException
    {
        String strMessage = "";
        ...
        if (Util.userCanAccess (899) == 0)
        {
            ...
            Logging.error (strMessage);
            AlertBroker.sendAlert ("EOD-ARO-002", strMessage);
        }
        else
        {   
            int intToDate = getDateToRollTo(repository);
            
            if (EndOfDay.rollToDates (intToDate, 0, intToDate, 1, intToDate) < 1)
            {
                strMessage = "Roll Other Dates failed";
                Util.scriptPostStatus (strMessage);
                Logging.error (strMessage);
                AlertBroker.sendAlert ("EOD-ARO-003", strMessage);
            }
            else
            {
                strMessage = "Rolled Other Dates to " + format_date (intToDate);
