...
public class SaveHistoricalPrices implements IScript {
    ...
    public void execute(IContainerContext context) throws OException {
        ...
        try {
        	...
        	if (Table.isTableValid(tblHistIndexList) == 1) {
        		tblHistIndexList.destroy();
        	}
        }
	...
	void saveHistoricalPrices(int iToday) throws OException {
    	...
    	try {
    		...
    		if (Table.isTableValid(allDayCurves) == 1) {
				allDayCurves.destroy();
			}
    	}
	}
	...
	private Table getAllDayCurves(Table tbl) throws OException {
    	...
    	sql = "SELECT index_id, 0 currency, 0 holiday_id, 0 holiday_date, 0 active, 0 id_number, 0 name, 0 weekend "
    			+ " FROM idx_def "
    			+ " WHERE idx_def.db_status = " + IDX_DB_STATUS_ENUM.IDX_DB_STATUS_VALIDATED.toInt();
    ...
    private void determineWeekendByHolidayCalendar(Table tbl) throws OException {
    	...
    	Table holidayList = Table.tableNew();
    	sql	= "SELECT * FROM holiday_list";
    	
    	try {
    		DBaseTable.execISql(holidayList, sql);
        	...
        	int rows = holidayList.getNumRows();
    		...
    		tbl.select(holidayList, "sun, mon, tue, wed, thu, fri, sat", "id_number EQ $id_number");
    		PluginLog.info("Weekken Day determined");
    		
    	} finally {
    		if (Table.isTableValid(holidayList) == 1) {
    			holidayList.destroy();
    		}
    	}
    }
