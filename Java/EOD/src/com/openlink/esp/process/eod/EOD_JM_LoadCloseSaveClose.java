...
public class EOD_JM_LoadCloseSaveClose implements IScript {
	...
	public void execute (IContainerContext context) throws OException {
		...
		try {
			...
			if (Table.isTableValid (tblLoadIndexList) == 1 && Table.isTableValid (tblSaveIndexList) == 1) {
			...
			if (Table.isTableValid (tblLoadIndexList) == 1) { 
				tblLoadIndexList.destroy ();
			}
			if (Table.isTableValid (tblSaveIndexList) == 1) { 
				tblSaveIndexList.destroy ();
			}
		}
	...
	void saveIndexClose(Table tblIndexList, int intToday) throws OException {
		PluginLog.debug ("Starting save closing data for " 
		...
		for (i = 1; i <= intNumRows; i++) {
			Table tblIdxGpts = Util.NULL_TABLE;
			try {
				// Get the index
				intIndexID = tblIndexList.getInt (intIdCol, i);
				strIndexFTD = "#" + intIndexID + " " + Table.formatRefInt (intIndexID, SHM_USR_TABLES_ENUM.INDEX_TABLE);
				...
				//Clean up
				if (Table.isTableValid(tblIdxGpts) == 1) {
					tblIdxGpts.destroy ();
				}
			}
		}
	...
	void saveVolaClose (String strDate) throws OException {
		...
		if (Table.isTableValid (tblVolaList) == 0) {
		...
		try {
			...
			strMessage = "Looping through " + intNumRows + " vola indexes";
			PluginLog.info (strMessage);
			...
			for (i = 1; i <= intNumRows; i++) {
				...
				strVolaFTD = "#" + intIndexID + " " + Table.formatRefInt (intIndexID, SHM_USR_TABLES_ENUM.VOLATILITY_TABLE);
	...
	private void saveCorrelationsClose(String dateToday) throws OException {
		...
		if (Table.isTableValid (correlationsToProcess) == 0) {
		...
		try {
			...
			if (Table.isTableValid (correlationsToProcess) == 1) {
