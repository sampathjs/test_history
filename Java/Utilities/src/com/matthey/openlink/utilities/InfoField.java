...
*/          
            if (infoData.getRowCount()>1) {
                ...
                for (int infoFieldItem = 0; infoFieldItem < infoData.getRowCount(); infoFieldItem++) {
                    ...
                    Logging.debug(String.format("Field >%s< is valid for Toolset:%d",infoFieldName,toolsets[infoFieldItem]));
                }
          ...
          Logging.error( "Field>" + infoFieldName + "< ecountered unexpected failure:"  + e.getLocalizedMessage(), e);
            e.printStackTrace();
            ...
            try {
              ...
              Logging.error( "Unexpected problem disposing work table:" + e.getLocalizedMessage(), e);
            }
        }
        
        Logging.debug( String.format("Field >%s< is %s InfoField",infoFieldName,null == field ? "NOT valid" : "valid"));
        return field;
        }catch(Exception e){
          throw new InfoFieldException("Field>" + infoFieldName + "< ecountered unexpected failure:" + e.getLocalizedMessage(), e);
        }finally{
