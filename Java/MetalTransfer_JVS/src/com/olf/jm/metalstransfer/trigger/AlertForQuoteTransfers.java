...
public class AlertForQuoteTransfers implements IScript {
	...
	public void execute(IContainerContext arg0) throws OException {
		Utils.initialiseLog(this.getClass().getName());	
		...
		try {
			...
			if (count <= 0) {
				...
				// Creating Email Body to be published
			 	eMailBody(body,subject,message,reportQuoteTransfers);
				String fileToAttach = com.matthey.utilities.FileNameFormatter.FileName(strFileName);
	...
	private void eMailBody(String body, String subject, String message, Table reportQuoteTransfers) throws OException {
		 
		try{
			
		 body = "Attached Transfer deals are still in Quote status booked through SAP interface for " 
				+ OCalendar.formatDateInt(Util.getTradingDate())
				+ ". Can you please look into them and take appropriate actions if required";
		 subject = "Transfer Deals in Quotes status for " + bUnit;
		message = "<html> \n\r"
	...
	private void fetchTPMVariable() throws OException {
		...
		try {
			...
			bUnit = com.matthey.utilities.Utils.getVariable(wflowId, "Int_Bunit");
	...
	private String fetchReciepents() throws OException {
		...
		String region = null;
		try {
			region = com.matthey.utilities.Utils.RegionToBU(bUnit);
			ConstRepository _constRepo = new ConstRepository("Strategy", "SapQuoteAlerts");
			recipient = _constRepo.getStringValue(region + "_QuoteReport");
