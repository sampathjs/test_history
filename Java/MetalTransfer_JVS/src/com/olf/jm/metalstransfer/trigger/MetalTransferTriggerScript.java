...
public class MetalTransferTriggerScript implements IScript {
	...
	public void execute(IContainerContext context) throws OException {
		...
		try {
			...
			for (int row = 1; row <= numRows; row++) {
				...
				Logging.info("Status updating to "+status+" for deal " + DealNum + " in USER_strategy_deals");
	...
	protected void init() throws OException {
		Logging.init(this.getClass(), "Strategy", "NewTrade");
		
		this.tpmToTrigger = constRepo.getStringValue("tpmTotrigger");
		if (this.tpmToTrigger == null || "".equals(this.tpmToTrigger)) {
			throw new OException("Invalid TPM defination in Const Repository");
		}
	...
	protected List<Integer> getCashDeals(int dealNum) throws OException {
		...
		try {
			...
			Logging.error("Failed to retrieve data for startegy " + dealNum + exp.getMessage());
		} finally {
