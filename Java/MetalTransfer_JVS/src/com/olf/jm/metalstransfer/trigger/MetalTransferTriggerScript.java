...
public class MetalTransferTriggerScript implements IScript {
		...
		try {
			...
			if (numRows == 0) {
				...
				for (int row = 1; row <= numRows; row++) {
					...
					PluginLog.info("Retrieved values for strategy, Deal: " + DealNum + ", UserId: " + userId
	...
	protected Table fetchStrategyDealsFailed() throws OException {
		...
		try {
			...
			if (ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt()) {
				PluginLog.error(DBUserTable.dbRetrieveErrorInfo(ret, "Failed while executing query for fetchStrategyDealsFailed method"));
				throw new OException("Failed while executing query for fetchStrategyDealsFailed method");
			}
			
		} catch (OException exp) {
			PluginLog.error("Error while fetching failed strategy Deals " + exp.getMessage());
			throw exp;
		}
	...
	protected String processTranNoCashTrade(int tranNum, int userId, String bUnit, String userName, String name) throws OException {
		...
		PluginLog.info("TPM triggered for strategy " + tranNum + " with workflowId " + this.workflowId);
	...
	protected String cancelOrDeleteCashTrades(List<Integer> cashDealList) throws OException {
		...
		for (int rowCount = 0; rowCount < countCash; rowCount++) {
			...
			PluginLog.info("Cancellation/Deletion process started for " + tranNum);
			...
			try {
				...
				if (status == TRAN_STATUS_ENUM.TRAN_STATUS_NEW.toInt()) {
					PluginLog.info("Deleting cash trade " + tranNum);
					Transaction.delete(tranNum, version);
					PluginLog.info("Deleted cash trade " + tranNum);
					
				} else if (status == TRAN_STATUS_ENUM.TRAN_STATUS_VALIDATED.toInt()) {
					PluginLog.info("Cancelling cash trade " + tranNum);
					Transaction.cancel(tranNum, version);
					PluginLog.info("Cancelled cash trade " + tranNum);
				}
				tradesProcessed++;
				
			} catch (OException exp) {
				PluginLog.error("Error while cancelling/deleting Cash deal "+ tranNum +" against Strategy, Error: " 
							+ exp.getMessage());
	...
	protected List<Integer> getCashDeals(int dealNum) throws OException {
		...
		try {
			if (dealNum > 0) {
				int cashDealCount;
				PluginLog.info("Retrieving Cash Deals for strategy " + dealNum);
				...
				if (ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt()) {
					PluginLog.error(DBUserTable.dbRetrieveErrorInfo(ret, "Failed while retrieving Cash Deals for strategy " + dealNum));
					throw new OException("Failed while retrieving Cash Deals for strategy " + dealNum);
				}
				
				cashDealCount = cashDealsTbl.getNumRows();
				PluginLog.info("Number of Cash deals returned " + cashDealCount +" for strategy " + dealNum);
				for (int row = 1; row <= cashDealCount; row++) {
					cashDealList.add(cashDealsTbl.getInt("tran_num", row));
				}
			}
		} catch (Exception exp) {
			PluginLog.error("Failed to retrieve cash trades for strategy " + dealNum + ", Error: " + exp.getMessage());
			throw exp;
	...
	protected Table strategyValidatedButPending() throws OException {
		...
		try {
			...
			PluginLog.error("Error in strategyValidatedButPending method: " + exp.getMessage());
			throw exp;
		}
	...
	protected Table fetchStrategyDeals() throws OException {
		...
		try {
			...
			if (ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt()) {
				PluginLog.error(DBUserTable.dbRetrieveErrorInfo(ret, "Failed while fetching Strategy deals for cash deal generation"));
			...
			PluginLog.error("Error while fetching strategy Deals, Error: " + exp.getMessage());
			throw exp;
