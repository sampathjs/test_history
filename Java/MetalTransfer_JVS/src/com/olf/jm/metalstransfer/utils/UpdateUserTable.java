...
// Stamp status in USER_Strategy_Deals	
	...
	public static void stampStatus(Table tbldata, int TranNum, int row, String status, int actualCashDeals,
			int expectedCount, int workflowId, String isRerun) throws OException {
		...
		try {
			...
			if (status.equalsIgnoreCase("Pending")
					|| status.equalsIgnoreCase("Succeeded") && isRerun.equalsIgnoreCase("No")) {
				retry_count++;
				Logging.info(TranNum + " is re-processed, therefore retry count is updated to " + retry_count
						+ " in user_strategy_deals");
			}
			...
			Logging.info("For strategy " + TranNum + " processed under TPM workflow ID " + workflowId + ", status ="
					+ status + " and trans_status = " + latestTranStatus + " and  actualCashDeals = " + actualCashDeals
					+ " and  expectedCount = " + expectedCount + " is updated at " + extractDateTime + "\n");
		} catch (OException oe) {
