...
public class Migr_StrategiesWithTranInfo_Creator_Plugin extends ApplicationScript implements IScript{
	public static final String COL_LENTITY = "int_lentity";
	public static final String COL_BUNIT = "int_bunit";
	...
	public void execute(IContainerContext context) throws OException {
		...
		*/
		...
		String name = "USER_migr_strategies";
		try {
			process(name);
		} catch (Exception e) {
		...
		for (int i = 1; i <= numRows; i++) {
			try {
				...
				if (table.getInt("row_type", i) == 2) {
					...
					//wait 5 secs 
					try { 
						Thread.sleep(1500);	// number in milliseconds
