...
public class RunReport extends AbstractTradeInputScript {
	...
	public Table execute(Context context, Transactions transactions) {
		...
		for (Transaction transaction : transactions) {
			...
			if (null == fromAccount || "".equalsIgnoreCase(fromAccount) 
				...
				String accountId = "";
				Table accountLookup = null;
				Table answer = null;
				
				try {
					accountLookup = DataAccess.getDataFromTable(context, 
							String.format("SELECT account_id FROM account WHERE account_name='%s'", fromAccount.trim()));
					
					if (null != accountLookup && accountLookup.getRowCount() == 1) {
						accountId = String.valueOf(accountLookup.getInt("account_id", 0));
					}
					
					Logging.info(String.format("From %s>%s<, %s, in %s  on %s",
									accountId,fromAccount, metalCode, unit, settleDate.getDisplayString()));
					...
					params.put("ReportAccount", accountId);
					...
					answer = processRBprocessCustomRequest(context, null, params);

					if (null != answer && answer.getRowCount() == 1)
						settleDateBalance = answer.getDouble("balance", 0);
					
					Logging.info(String.format("Xfer From %s balance(%f) on SettleDate %s in %s",
									accountId, settleDateBalance, settleDate.getValueAsString(), unit));
	...
	private Table processRBprocessCustomRequest(Context context, Table argumentsTable, Map<String, String> parameters) {
		...
		try {
			...
			Logging.error("Error in executing report", e);
		}
		...
		if (result < 1) {
			...
			try {
				...
				Logging.error("\t Report Dispatcher error cloning result!", e);
			}
			return output;
		}
	}
