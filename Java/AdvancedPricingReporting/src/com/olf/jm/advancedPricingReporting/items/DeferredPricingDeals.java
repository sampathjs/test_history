...
public class DeferredPricingDeals extends ItemBase {
	...
	private Table loadDispatchDeals(int customerId, Date matchDate, int metalType, double hkUnitConversion) {
		...
		sql.append("        JOIN (SELECT Sum(total_quantity) AS total_vol, \n");
		sql.append("                     param_seq_num, \n");
		sql.append("                     ins_num \n");
		sql.append("              FROM   comm_schedule_header csh \n");
		sql.append("              WHERE  volume_type = 4 \n");
		sql.append("              GROUP  BY param_seq_num, \n");
		sql.append("                        ins_num) AS vol \n");
		sql.append("          ON vol.ins_num = p.ins_num \n");
		sql.append("             AND p.param_seq_num = vol.param_seq_num \n");
