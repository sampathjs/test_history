...
public class LoanDep extends AbstractEndurDealExtract {
    ...
    protected Table getData() throws OException {
        ...
        tblLoanDep.addCol("position_toz", COL_TYPE_ENUM.COL_DOUBLE);
    
        int numRows = tblLoanDep.getNumRows();
        int toz = Ref.getValue(SHM_USR_TABLES_ENUM.IDX_UNIT_TABLE, Constants.TROY_OUNCES);
        for (int row = 1; row <= numRows; row++)
        {
            int metalUnit = tblLoanDep.getInt("metal_unit", row);
            double metalPosition = tblLoanDep.getDouble("position_metal_unit", row);
            ...
            tblLoanDep.setDouble("position_metal_unit", row, metalPosition);
            tblLoanDep.setDouble("position_toz", row, metalPositionToz);
        }
