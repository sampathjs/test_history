...
public class TableSplitter_Mode2 extends AbstractGenericScript {
	...
	private Table process(final Session session) {
    	...
    	for (int i=0; i < outputTableCount; i++) {
    		...
    		try {
    			PluginLog.info("Trying to clear data in user table " + tableName);   			
    			session.getIOFactory().getUserTable(tableName).clearRows();  			    			
    			PluginLog.info("Successfully clear all data in user table " + tableName);
    		} catch (RuntimeException ex) {
    			PluginLog.warn("Could not clear user table " + tableName);
    		}
    	}
    	...
    	for (int i=0; i < outputTableCount; i++) {
			PluginLog.info("Insert data into user table " + dstTables[i].getName());
