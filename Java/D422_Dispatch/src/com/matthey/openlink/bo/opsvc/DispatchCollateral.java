...
public class DispatchCollateral {
	...
	public static double evaluate(Session session, Transaction transaction, List<SettlementInstruction> settlements) {
		...
		try {
			...
			Logging.info(String.format("%s:Elapsed %dms", "Collateral Evaluation",
	...
	private double result() {
		...
		Logging.info(String.format("%s Balance:%f Collateral:%f", this.getClass().getSimpleName(), balance,
				collateralBalance));
...
//		}
		...
		if(Double.isNaN(collateralBalance)) {
			Logging.info(String.format("No Collateral Account for Tran#%d",
...
//			return; 
	...
	private double retrieveAccountBalance(Session session, Map<String, String> parameters, String task) {
		...
		Logging.info(String.format("%s Balance:%f", parameters.get("account"), result));
		return result;
	}
