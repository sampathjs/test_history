...
/**
 * This class runs a script that will check, for all ComSwap, if, for today's
 * reset, a historical price is available, and if so, will create an entry for
 * that reset in the USER_JM_PNL_MARKET_DATA table and USER_JM_JDE_EXTRACT
 * table.
 * 
 */
public class PNL_Handle_Intraday_Fixing implements IScript {
	...
	public void execute(IContainerContext context) throws OException {
		...
		// Retrieve all ComSwap transactions for which we may need to store the
		// intraday fixing data
		...
		// Process the data - upload their market data to
		// USER_jm_pnl_market_data and USER_JM_JDE_EXTRACT
	...
	private void prepareUserTableData(Table transactions) throws OException {
		// Retrieve all resets for which we have already stored information in
		// USER_JM_PNL_MARKET_DATA
		Vector<PNL_MarketDataEntry> knownResets = PNL_UserTableHandler
				.retrieveMarketData(transactions);
	...
	/**
	 * Given a set of transactions and their current data entries, save only the
	 * non-existing ones to USER table
	 * 
	 * @param transactions
	 * @param dataEntries
	 * @throws OException
	 */
	private void processDataEntries(Table transactions,
			Vector<PNL_MarketDataEntry> dataEntries) throws OException {
		// Add all entries to DB
		PluginLog.info("PNL_Handle_Intraday_Fixing found " + dataEntries.size()
				+ " new entries. Inserting\n");
		OConsole.message("PNL_Handle_Intraday_Fixing found "
				+ dataEntries.size() + " new entries. Inserting\n");
	 ...
	 * @throws OException
	 */
	private Vector<PNL_MarketDataEntry> prepareMarketData(Table transData)
			throws OException {
		Vector<PNL_MarketDataEntry> allDataEntries = new Vector<PNL_MarketDataEntry>();

		// Refresh historical prices, as this script is run after the historical
		// prices have been loaded
		MTL_Position_Utilities.refreshHistPrices();

		try {
			// For each ComSwap, retrieve relevant fixings for that date, and
			// add to overall list
			for (int row = 1; row <= transData.getNumRows(); row++) {
				Transaction trn = transData.getTran("tran_ptr", row);

				Vector<PNL_MarketDataEntry> dataEntries = PNL_FixingsMarketDataRecorder
						.processDeal(trn);

				// For each data entry, let's check -
				// Does its relevant historical price already exist?
				// Is there no entry for it in the user-jm-pnl-market-data
				// table?

				for (PNL_MarketDataEntry entry : dataEntries) {
					boolean shouldAdd = true;

					// Check if this already has an entry in
					// USER_JM_PNL_MARKET_DATA
					...
					// Retrieve the relevant parameters from the deal-leg-reset
					// to see if it has a known historical price
					int leg = entry.m_uniqueID.m_dealLeg;
					int resetID = entry.m_uniqueID.m_dealReset >= PNL_FixingsMarketDataRecorder.S_FX_RESET_OFFSET ? entry.m_uniqueID.m_dealReset
							- PNL_FixingsMarketDataRecorder.S_FX_RESET_OFFSET
							: entry.m_uniqueID.m_dealReset;

					int projIdx = trn.getFieldInt(
							TRANF_FIELD.TRANF_PROJ_INDEX.toInt(), leg);
					int resetDate = trn.getFieldInt(
							TRANF_FIELD.TRANF_RESET_DATE.toInt(), leg, "",
							resetID);
					int rfisDate = trn.getFieldInt(
							TRANF_FIELD.TRANF_RESET_RFIS_DATE.toInt(), leg, "",
							resetID);
					int refSource = trn.getFieldInt(
							TRANF_FIELD.TRANF_REF_SOURCE.toInt(), leg);

					// Check if this reset has an entry in historical prices; if
					// no, we should not store its data
					if (!MTL_Position_Utilities.hasHistPrice(projIdx,
							resetDate, rfisDate, refSource)) {
						shouldAdd = false;
					}
			...
			String msg = "PNL_Handle_Intraday_Fixing.prepareMarketData threw exception: "
					+ e.getMessage() + "\n";
			PluginLog.info(msg);
			OConsole.message(msg);
		}

		String msg = "PNL_Handle_Intraday_Fixing.prepareMarketData found "
				+ allDataEntries.size() + " entries to process.\n";
		PluginLog.info(msg);
		OConsole.message(msg);
	...
	/**
	 * Prepare the list of transactions of interest, and initialise
	 * m_indexLoadTable from those
	 * 
	 * @param transData
	 * @throws OException
	 */
	private void prepareTransactionData(Table transData) throws OException {
		...
		int liborIndex = Ref.getValue(SHM_USR_TABLES_ENUM.INDEX_TABLE,
				"LIBOR.USD");
		...
		// Add all PX_ Metals indices, as this will force refresh prices on all
		// child indexes that ComSwaps price off
		String[] metals = new String[] { "XAG", "XAU", "XIR", "XAG", "XOS",
				"XPD", "XPT", "XRH", "XRU" };
		String metalIndexFormat = "PX_<METAL>.USD";
		for (String metal : metals) {
			int metalIdx = Ref.getValue(SHM_USR_TABLES_ENUM.INDEX_TABLE,
					metalIndexFormat.replace("<METAL>", metal));

			if (metalIdx > 0)
				indexesToLoad.add(metalIdx);
		}

		for (int row = transData.getNumRows(); row >= 1; row--) {
			Transaction trn = transData.getTran("tran_ptr", row);

			boolean bTodayResetFound = false;

			int numParams = trn.getNumRows(-1,
					TRANF_GROUP.TRANF_GROUP_PARM.toInt());
			for (int param = 0; param < numParams; param++) {
				int fxFlt = trn.getFieldInt(TRANF_FIELD.TRANF_FX_FLT.toInt(),
						param);

				// Skip the fixed (deliverable) swap leg, we only store resets
				// from floating legs
				if (fxFlt == fixedLeg) {
					continue;
				}

				int totalResetPeriods = trn.getNumRows(param,
						TRANF_GROUP.TRANF_GROUP_RESET.jvsValue());

				for (int reset = 0; reset < totalResetPeriods; reset++) {
					int blockEnd = trn.getFieldInt(
							TRANF_FIELD.TRANF_RESET_BLOCK_END.jvsValue(),
							param, "", reset);
					...
					int resetDate = trn.getFieldInt(
							TRANF_FIELD.TRANF_RESET_DATE.jvsValue(), param, "",
							reset);
				...
				// Retrieve projection index - we will need to load its closing
				// prices
				int projIdx = trn.getFieldInt(
						TRANF_FIELD.TRANF_PROJ_INDEX.toInt(), param, "", 0, 0);
				indexesToLoad.add(projIdx);

				// If projection index currency doesn't match leg currency, we
				// will have to convert FX, so need to load FX indexes too
				int legCcy = trn.getFieldInt(
						TRANF_FIELD.TRANF_CURRENCY.toInt(), param);
				if (MTL_Position_Utilities.getPaymentCurrencyForIndex(projIdx) != legCcy) {
					indexesToLoad.add(MTL_Position_Utilities
							.getDefaultFXIndexForCcy(legCcy));
				}
			}

			// If this transaction does not have any resets today, delete it
			// from the list
			if (!bTodayResetFound) {
		...
		PluginLog
				.info("PNL_Handle_Intraday_Fixing.prepareTransactionData found "
						+ indexLoadTable.getNumRows() + " indexes to load.\n");
		OConsole.message("PNL_Handle_Intraday_Fixing.prepareTransactionData found "
				+ indexLoadTable.getNumRows() + " indexes to load.\n");
	...
	private Table retrieveRelevantTransactions() throws OException {
		PluginLog
				.info("PNL_Handle_Intraday_Fixing.retrieveRelevantTransactions\n");
		...
		String sql = "SELECT ab.deal_tracking_num deal_num, ab.tran_num FROM ab_tran ab "
	...
	public void process(Transaction trn, HashSet<Integer> dates)
			throws OException {
	...
	private void initPluginLog() throws OException {
		...
		PluginLog
				.info("Plugin: " + this.getClass().getName() + " started.\r\n");
	}
}
