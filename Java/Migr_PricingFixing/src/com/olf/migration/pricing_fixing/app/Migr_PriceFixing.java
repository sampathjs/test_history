 ...
 */
public class Migr_PriceFixing extends Migr_PriceFixing_Common implements IScript
{
	/**
	...
	public static final String LOG_FILE = "Migr_PricingFixing.log";

	/**
	 * The last day the default value is applied to. Format has to be parseable by standard OLF date parser.
	 */
	public static final String END_DATE = "27/08/2016";
			...
			while (group != null) {
				...
				for (int row=1; row <= rowCount; row++) {
					...
					if (tran.getNumRows() > 0) {
						...
						Table result = EndOfDay.resetDealsByTranList(tran, resetDate);
				...
				if (resetList != null && resetList.getNumRows() > 0) {
					if (VIEW_TABLE) {
						resetList.viewTable();						
					}
					if (SAVE_CSV_REPORT) {
	...
	protected String getSql() throws OException {
		...
		String sql =       "\nSELECT DISTINCT ab.tran_num, r.reset_date, ab.internal_portfolio"
					+	   "\nFROM " + resultTable + " qr"
