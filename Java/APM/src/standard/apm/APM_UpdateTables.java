...
import standard.apm.APM_DataStoreOps;
...
public class APM_UpdateTables implements IScript {
		...
		try {
			...
			for (iRow = 1; (iRetVal == 1 && (iRow <= tPackageDetails.getNumRows())); iRow++) 
			{
				...
				if (iLoopRetVal != 0) 
					...
					m_APMUtils.APM_PrintAndLogErrorMessage(iMode, argt, "APM reporting table update failed for tranche #" + argt.getInt("Tranche", 1));
			...
			if ( iRetVal == 0 ) {
				try {
					...
					m_APMUtils.APM_PrintAndLogErrorMessage( iMode, argt, exception.toString());
				}
			} else {
	...
	throws OException 
	{
		...
		if ( useADS) //FOR ADS DO A SINGLE BLOCK INSERT
		{
	...
	int UpdateTablesForEntityGroupAndPackage(int iMode, int entityGroupId, String sPackageName, int iDatasetTypeId, int serviceID, boolean datasetKeyInAnotherService, String sJobName, Table tMainArgt, Table tPackageDataTables, Table argt) throws OException {
		...
		if (iRetVal != 0) {
			...
			m_APMUtils.APM_PrintAndLogErrorMessage(iMode, argt, sProcessingMessage);
		}
