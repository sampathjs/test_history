...
import com.olf.apm.interfaces.configuration.IDbConfiguration;
import com.olf.apm.ioc.Container;

public class ADA_DataStoreOps
{
	...
	private static ADA_DataStoreOps self;
	
	private ADA_DataStoreOps() {
		m_APMUtils = new APM_Utils();
		m_db = (IDataSink) Container.instance().getInstance("IDataSink");
	}

	public static ADA_DataStoreOps instance() throws OException {
		if (self == null) {
			self = new ADA_DataStoreOps();
		}
	...
	public int Update(int iUpdateMode, int iMode, int entityGroupId, String sPackageName, int datasetType, int serviceID, boolean datasetKeyInAnotherService, String sJobName, Table tMainArgt, Table tPackageDataTables, Table argt) throws OException 
	{
		...
		try {
			...
			if (iUpdateMode == m_APMUtils.APM_BACKOUT) {						   
					...
					while (!complete && pruningHandler.Elapsed() <= m_lockThreadTotalSleepTime) {								
						try {				
							...
							if (locked) {
								...
								if ( entities.size() > 0 ) {
									iRetVal = m_db.backoutEntity(argt, cacheName, primaryEntityNum);
				...
				iRetVal = m_db.putAllPending(argt, dataTable);
			} 
					...
					try {										
						...
						if (locked) {						
							...
							if ( used.size() > 0 ) {
								...
								iRetVal = m_db.applyBlockUpdate(argt, dataTable, entities);
