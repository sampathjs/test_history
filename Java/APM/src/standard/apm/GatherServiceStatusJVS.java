...
public class GatherServiceStatusJVS implements IScript {
	...
	public GatherServiceStatusJVS() throws OException
	{
		try {
			...
			print("GatherServiceStatus static constructor, ADSException " + stackTraceToString(e));
		} catch (InstantiationException e) {
			print("GatherServiceStatus static constructor, InstantiationException " + stackTraceToString(e));			
		} catch (IllegalAccessException e) {
			print("GatherServiceStatus static constructor, IllegalAccessException " + stackTraceToString(e));			
		} catch (ClassNotFoundException e) {
			print("GatherServiceStatus static constructor, ClassNotFoundException " + stackTraceToString(e));			
		}
...
//	   print("after Login");
	   ...
	   m_table = context.getArgumentsTable();
   ...
   private static void print(String message) {
   	...
   	try {
	   OConsole.oprint(OCalendar.formatDateInt(OCalendar.getServerDate(), DATE_FORMAT.DATE_FORMAT_DMLY_NOSLASH) + " " + Util.timeGetServerTimeHMS() + ":[SCRIPT:GatherServiceStatusJVS] => " + message + "\n");
   	}
   	catch (Exception ex)
   	{
	   ...
	   for (int serviceRow = serviceStatus.getNumRows(); serviceRow > 0; serviceRow--) {
		   ...
		   if (serviceGroupText.equals("APM") || serviceGroupText.equals("APM Nomination")) {
			   ...
			   for (int engineRow = 1; engineRow <= engineTable.getNumRows(); engineRow++) {
				   ...
				   // }
				   if (engineName.contains("ApmService") || engineName.contains("ApmNomService")) {
					   try {
   ...
   protected Table GetOverlaysForMethod(String serviceName, String sMethod) throws OException {
	   ...
	   // dataset is present - so no need to use the user table
	   if ( datasetPresent.getNumRows() > 0 )
	   {
   ...
   protected Table GetMselTable(Table methodConfig, String valueName) throws OException {
		...
		if (propRow > 0) {
			...
			tMselValues = m_table.tableNew("msel_values");
	...
	private Map<Map, Map> getAllDatasetKeys(int service, Map serviceInfo) throws OException {
		...
		if (datasetDetailsTable != null) {
			for (int dsRow = 1; dsRow <= datasetDetailsTable.getNumRows(); dsRow++) {
				...
				dsKeyDetails.put("$dataset_type_name$", datasetDetailsTable.getString("dataset_type_name", dsRow));
