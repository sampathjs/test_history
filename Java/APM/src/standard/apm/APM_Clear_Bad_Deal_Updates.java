...
public class APM_Clear_Bad_Deal_Updates implements IScript 
{
	...
	//
	public APM_Clear_Bad_Deal_Updates() 
	{
	...
	public void execute(IContainerContext context) throws OException 
	{
		...
		try
		{
			...
			m_apmUtils.APM_PrintMessage(tAPMArgumentTable, "------- CLEARING BAD DEAL UPDATES !!! --------");			
			...
			if ( argt.getColNum("ServiceName") > 0 && argt.getNumRows() > 0 )
			{
				for (int i = 1; i <= argt.getNumRows(); i++)
				{
					...
					deleteBadDealUpdatesForService(sServiceName, tAPMArgumentTable);
				}
			}
			else
			{
				m_apmUtils.APM_PrintMessage(tAPMArgumentTable, "------- NO SERVICE NAMES IN ARGT: CLEARING BAD DEAL UPDATES FOR ALL SERVICES --------");
			...
			m_apmUtils.APM_PrintMessage(tAPMArgumentTable, "------- FINISHED CLEARING BAD DEAL UPDATES --------");
		...
		{
			returnValue = 0;
			OConsole.oprint( "Exception while calling Clear Bad Deal Updates script: " + t);
	...
	private void InitialiseArgTable(Table tAPMArgumentTable) throws OException
	{
		...
		tAPMArgumentTable.addCol("service", COL_TYPE_ENUM.COL_INT);
		tAPMArgumentTable.addCol("service_name", COL_TYPE_ENUM.COL_STRING);
		tAPMArgumentTable.addCol("dataset_type", COL_TYPE_ENUM.COL_INT);
		...
		tAPMArgumentTable.addCol("Portfolio Ids", COL_TYPE_ENUM.COL_TABLE);
		tAPMArgumentTable.addCol("Bad Deal Updates", COL_TYPE_ENUM.COL_TABLE);
		tAPMArgumentTable.addCol("Main Argt", COL_TYPE_ENUM.COL_TABLE);
		tAPMArgumentTable.addCol("Portfolio_Scenario_List", COL_TYPE_ENUM.COL_TABLE);
		...
		Table tPortfolioScenarioList = Table.tableNew("scenario_list");
		tPortfolioScenarioList.addCol("scenario_id", COL_TYPE_ENUM.COL_INT);
		tPortfolioScenarioList.addCol("scenario_name", COL_TYPE_ENUM.COL_STRING);
		tPortfolioScenarioList.addNumRows(1);
		tAPMArgumentTable.setTable("Portfolio_Scenario_List", 1, tPortfolioScenarioList);
		
		tAPMArgumentTable.setInt("Debug", 1, 1);

		StatisticsLogging.instance().setProcessIdContext(tAPMArgumentTable, Ref.getProcessId());

		StatisticsLogging.instance().setScriptContext(tAPMArgumentTable, "APM_Clear_Bad_Deal_Updates");
	}
	...
	//
	private int deleteBadDealUpdatesForService(String sServiceName, Table tAPMArgumentTable) throws OException 
	{
		int iRetVal = 0;

		m_apmUtils.APM_PrintMessage(tAPMArgumentTable, "Attempting to clear Bad Deal Updates for Service: " + sServiceName);
		
		int iServiceID = tAPMArgumentTable.getInt("service", 1);
		
		Table tBadPortfolios = Table.tableNew("bad_portfolios");
	   int returnValue = m_apmUtils.APM_TABLE_LoadFromDbWithSQL(tAPMArgumentTable, tBadPortfolios, "distinct portfolio, rtp_page_subject, dataset_type, scenario", "tfe_msg_log", "service = " + iServiceID + " and msg_type = 2 and dealnum > -1");
		if (returnValue != DB_RETURN_CODE.SYB_RETURN_SUCCEED.toInt() )
		{
			m_apmUtils.APM_PrintErrorMessage(tAPMArgumentTable, "Unable to run SQL to find Portfolios containing Bad deal updates for APM service.");
			tBadPortfolios.destroy();
			return 0;			
		}
		
		if (tBadPortfolios.getNumRows() == 0 )
		{
			m_apmUtils.APM_PrintMessage(tAPMArgumentTable, "Nothing to do.....");
			return 1;
		}

		returnValue =  m_apmUtils.APM_FindbadDealUpdates(m_apmUtils.cModeBatch, iServiceID, tAPMArgumentTable, Util.NULL_TABLE);
		if ( returnValue == 0)
		{
			m_apmUtils.APM_PrintErrorMessage(tAPMArgumentTable, "Failed to find bad deal updates for service:" + sServiceName);
			return 0;
		}
				
		for (int i = 1; i <= tBadPortfolios.getNumRows(); i++)
		{
			int iPortfolioId = tBadPortfolios.getInt(1, i);
			// set the rtp page subject into the user table
			int endStringPosition = Str.findSubString(tBadPortfolios.getString(2, i), ".");
			String prefix = Str.substr(tBadPortfolios.getString(2, i), 0, endStringPosition + 1); // include the dot
			tAPMArgumentTable.setString("RTP Page Prefix", 1, prefix);
			
			// set the dataset type
			int datasetType = tBadPortfolios.getInt(3, i);
			tAPMArgumentTable.setInt("dataset_type", 1, datasetType);
			String sDatasetType = m_apmUtils.APM_DatasetTypeIdToName(tAPMArgumentTable, datasetType, false);
			if (Str.len(sDatasetType) < 1)
			   sDatasetType = Str.intToStr(datasetType);

			StatisticsLogging.instance().setDatasetTypeContext(tAPMArgumentTable, sDatasetType, datasetType);
			
			// set the scenario ID
			int scenario = tBadPortfolios.getInt(4, i);
			Table tPortfolioScenarioList = tAPMArgumentTable.getTable("Portfolio_Scenario_List", 1);
			tPortfolioScenarioList.setInt(1, 1, scenario);
			// we don't have access to the scenario name here so just convert the ID to a string
			StatisticsLogging.instance().setScenarioContext(tAPMArgumentTable, Str.intToStr(scenario), scenario);

			returnValue =  m_apmUtils.APM_ClearTFEMsgLogForBadDealUpdates(m_apmUtils.cModeBatch, iPortfolioId, scenario, "", tAPMArgumentTable);
			if ( returnValue == 0)
			{
				m_apmUtils.APM_PrintErrorMessage(tAPMArgumentTable, "Failed to clear bad deal updates for portfolio ID:" + Table.formatRefInt(iPortfolioId, SHM_USR_TABLES_ENUM.PORTFOLIO_TABLE) +
																						", Dataset type: " + datasetType + ", Scenario: " + scenario);
				m_apmUtils.APM_PrintMessage(tAPMArgumentTable, "Continuing.....");
				iRetVal = 0;
			}
		}

		StatisticsLogging.instance().unSetDatasetTypeContext(tAPMArgumentTable);
		StatisticsLogging.instance().unSetScenarioContext(tAPMArgumentTable);
