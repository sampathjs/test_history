...
*/
	...
	private Table retrieveFxSwapData(Table tblEventData) throws OException
	{
		...
		try
		{
			...
			catch (OException e) { tblTranGroup.printTable(); throw e; }
			finally { tblTranGroup.destroy(); }
		}
		catch (OException e) { OConsole.print("\n"+e.toString()); }
	...
	private void loadFromDB(Table tblTranGroup, Table tblResult) throws OException
	{
		...
		if (tblTranInfoType.getNumRows() > 0)
		{
			...
			if (row > 0) iTranInfoIdForm = tblTranInfoType.getInt(1, row);
			row     = tblTranInfoType.findString(2, TRAN_INFO_LOCO, SEARCH_ENUM.FIRST_IN_GROUP);
			if (row > 0) iTraninfoIdLoco = tblTranInfoType.getInt(1, row);
		}
				...
				//	tblFxTranAuxData.viewTable();
					...
					try
					{
						...
						try
						{
							...
							finally { tblAbTranInfo.destroy(); }
						}
						finally { tblCurrency.destroy(); }
					}
					finally { tblCcys.destroy(); }
				}
				finally { tblFxTranAuxData.destroy(); }
			}
			finally { tblTranNum.destroy(); }
		}
		finally { tblAbTran.destroy(); }
	}
}
