...
public class JM_MOD_FXSwap extends OLI_MOD_ModuleBase implements IScript {
	...
	private void createItemsForSelection(Table itemListTable) throws OException {
		...
		/*
		 * // copied from groupName = "FX Trade Info";
		 * ItemList.add(itemListTable, groupName, "FX Date", "olfFxDate", 1);
		 * groupName = "FX Trade Info, Corporate Margin";
		 * ItemList.add(itemListTable, groupName, "Inter Or Corp",
		 * "olfFxInterOrCorp", 1); ItemList.add(itemListTable, groupName,
		 * "Corp Margin", "olfFxCorpMargin", 1);
		 */
	...
	private void retrieveGenerationData() throws OException {
		...
		// {
		// PluginLog.error
		// ("Unable to retrieve transaction info due to invalid transaction object found. Tran#"
		// + tranNum);
		// }
		// else
		{
			...
			String olfTblFxSwapData = null, olfFXSwapTotalAmount = null, olfFXSwapWeYou = null, olfNumValueDates = null;
			...
			for (row = 1; row <= numRows; row++) {
				...
				/*
				 * //FX Trade Info, FX Date else if
				 * (internal_field_name.equalsIgnoreCase("olfFxDate")) { String
				 * strValue = tran.getField(TRANF_FIELD.TRANF_FX_DATE.toInt());
				 * GenData.setField(gendataTable, output_field_name, strValue);
				 * }
				 * 
				 * //FX Trade Info, Corporate Margin, Inter Or Corp else if
				 * (internal_field_name.equalsIgnoreCase("olfFxInterOrCorp")) {
				 * String strValue =
				 * tran.getField(TRANF_FIELD.TRANF_FX_INTER_OR_CORP.toInt());
				 * GenData.setField(gendataTable, output_field_name, strValue);
				 * }
				 * 
				 * //FX Trade Info, Corporate Margin, Corp Margin else if
				 * (internal_field_name.equalsIgnoreCase("olfFxCorpMargin")) {
				 * String strValue =
				 * tran.getField(TRANF_FIELD.TRANF_FX_CORP_MARGIN.toInt());
				 * GenData.setField(gendataTable, output_field_name, strValue);
				 * }
				 */
	...
	private Table retrieveFxSwapData(Table tblEventData) throws OException {
		...
		try {
			tblFxSwapData.addCols(""
					// all columns come from 'ab_tran' - if not mentioned
					// otherwise
					+ "I(tran_num)" + "I(tran_group)" + "I(ins_type)" + "I(buy_sell)" + "S(reference)" + "I(internal_bunit)" + "I(internal_lentity)" + "I(external_bunit)"
					+ "I(external_lentity)" + "I(internal_portfolio)" + "I(external_portfolio)" + "T(trade_date)" + "T(settle_date)" + "F(position)" + "F(price)" + "F(rate)"
					+ "F(pymt_amount)" // = price * position
					+ "I(currency)" + "I(ccy1)" // from fx_tran_aux_data - to
												// become 'fx_ccy1' ?
					+ "S(ccy1_desc)" // from currency
					+ "I(ccy2)" // from fx_tran_aux_data - to become 'fx_ccy2' ?
					+ "S(ccy2_desc)" // from currency
					+ "I(cflow_type)" + "I(ins_sub_type)" + "T(start_date)" + "T(maturity_date)" + "I(unit)" + "I(template_tran_num)" + "S(form)" // from
																																					// ab_tran_info.value
			...
			try {
				...
				additionConversion(tblFxSwapData);
	...
	private void additionConversion(Table tblFxSwapData) {
		try {
			...
			String baseUnit = "TOz";
			int fromUnit = com.olf.openjvs.Ref.getValue(SHM_USR_TABLES_ENUM.IDX_UNIT_TABLE, baseUnit);
			Double position = tblFxSwapData.getDouble("position", 1);
			Double price = tblFxSwapData.getDouble("price", 1);
			int toUnit = tblFxSwapData.getInt("unit", 1);
			Double convFactor = Transaction.getUnitConversionFactor(fromUnit, toUnit);
			Double newPosition = position * convFactor;
			Double newPrice = price / convFactor;
			int ret = tblFxSwapData.setColValDouble("position", newPosition);
			if (ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt()) {
				PluginLog.error(DBUserTable.dbRetrieveErrorInfo(ret, "Failed while updating converted position in unit " + toUnit));
			}
			int retval = tblFxSwapData.setColValDouble("price", newPrice);
			if (retval != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt()) {
				PluginLog.error(DBUserTable.dbRetrieveErrorInfo(ret, "Failed while updating converted price per unit " + toUnit));
			}
		} catch (OException e) {
	...
	private void loadFromDB(Table tblTranGroup, Table tblResult) throws OException {
		...
		DBaseTable.execISql(tblTranInfoType, "select type_id, type_name from tran_info_types where type_name in ('" + TRAN_INFO_FORM + "','" + TRAN_INFO_LOCO
				+ "') order by type_name");
		...
		try {
			...
			tblAbTran.addCols("" + "I(tran_num)" + "I(tran_group)" + "I(ins_type)" + "I(buy_sell)" + "S(reference)" + "I(internal_bunit)" + "I(internal_lentity)"
					+ "I(external_bunit)" + "I(external_lentity)" + "I(internal_portfolio)" + "I(external_portfolio)" + "T(trade_date)" + "T(settle_date)" + "F(position)"
					+ "F(price)" + "F(rate)"
					// + "F(pymt_amount)" // = price * position
					+ "I(currency)"
					// + "I(ccy1)" // from fx_tran_aux_data - to become
					// 'fx_ccy1' ?
					// + "S(ccy1_desc)" // from currency
					// + "I(ccy2)" // from fx_tran_aux_data - to become
					// 'fx_ccy2' ?
					// + "S(ccy2_desc)" // from currency
					+ "I(cflow_type)" + "I(ins_sub_type)" + "T(start_date)" + "T(maturity_date)" + "I(unit)" + "I(template_tran_num)"
					...
					try {
						...
						try {
							...
							try {
								...
								tblResult.setColValInt("ccy1", -1);
								tblResult.setColValInt("ccy2", -1);
