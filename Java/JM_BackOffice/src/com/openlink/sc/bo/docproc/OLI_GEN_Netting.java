...
public class OLI_GEN_Netting implements IScript {
	...
	private void process(IContainerContext context) throws OException {
		...
		tbl.addCols("I(document_num)I(settle_ccy)I(settle_unit)F(settle_amount)F(saved_settle_amount)F(settle_amount_Invoice)I(amounts_differ)");
		...
		for (int row=0; ++row <= numRows; ) {
			...
			if (info)  {
				OConsole.oprint(ret==OLF_RETURN_SUCCEED?".":"f");
			}
			if (debug) {
				OConsole.oprint("\t"+tbl.getInt64(col_event_num, row)+"\t"+tbl.getDouble(col_settle_amount_Invoice, row)+"\t"+(ret==OLF_RETURN_SUCCEED?"done":ret)+"\n");
			}
			retTotal *= ret;
		}
		if (info) {
