...
//	private List<String> _final_invoice_statuses = null;//< ConstRep
	...
	private void process(IContainerContext context) throws OException
	{
		...
		else
		{
			...
			{
				tblHelp.select(tblEvent
					,"next_doc_status,buy_sell"
					,"doc_type EQ $doc_type_id AND internal_lentity EQ $our_le_id");
	...
	protected final String getNextDocNumber(Table tblDocNumberingData, boolean isPreview) throws OException
	{
		...
		{ throw new OException(e.getMessage()); }
		String item = ""+tblDocNumberingData.getInt("our_le_id", 1)
	...
	{
		_generated_statuses = Arrays.asList(
				tryRetrieveSettingFromConstRep(GENERATED_STATUS, _generated_status)
				.trim().replaceAll("\\s*,\\s*", ",").split(","));

		_cancelled_statuses = Arrays.asList(
				tryRetrieveSettingFromConstRep(CANCELLED_STATUS, _cancelled_status)
				.trim().replaceAll("\\s*,\\s*", ",").split(","));

		_prepared_statuses = Arrays.asList(
				tryRetrieveSettingFromConstRep(PREPARED_STATUS, _prepared_status)
				.trim().replaceAll("\\s*,\\s*", ",").split(","));
...
*/
		...
		if (id >= 0) { _stldoc_info_this_doc_num = name; /*_stldoc_info_this_doc_num_id = id;*/ }
		else if (name.trim().length()>0) throw new OException("StlDoc Info Field for feature '"+THIS_DOC_NUM+"' not found - invalid name: '"+name+"'");

		id = getStlDocInfoTypeId(name = tryRetrieveSettingFromConstRep(LAST_DOC_NUM, _stldoc_info_last_doc_num));
		if (id >= 0) { _stldoc_info_last_doc_num = name; /*_stldoc_info_last_doc_num_id = id;*/ }
		else if (name.trim().length()>0) throw new OException("StlDoc Info Field for feature '"+LAST_DOC_NUM+"' not found - invalid name: '"+name+"'");

		id = getStlDocInfoTypeId(name = tryRetrieveSettingFromConstRep(PREP_DOC_NUM, _stldoc_info_prep_doc_num));
		if (id >= 0) { _stldoc_info_prep_doc_num = name; /*_stldoc_info_prep_doc_num_id = id;*/ }
		else if (name.trim().length()>0) PluginLog.debug("StlDoc Info Field for feature '"+PREP_DOC_NUM+"' not found - invalid name: '"+name+"'");

		id = getStlDocInfoTypeId(name = tryRetrieveSettingFromConstRep(PROV_DOC_NUM, _stldoc_info_prov_doc_num));
		if (id >= 0) { _stldoc_info_prov_doc_num = name; /*_stldoc_info_prov_doc_num_id = id;*/ }
		else if (name.trim().length()>0) PluginLog.debug("StlDoc Info Field for feature '"+PROV_DOC_NUM+"' not found - invalid name: '"+name+"'");
	}
	...
	private void setOutputFields(Table tblOutput, int document_num, String this_doc_num, String last_doc_num, String prep_doc_num, String prov_doc_num) throws OException
	{
		...
		try
		{
			...
			if (dataType == STLDOC_GEN_DATA_TYPE.STLDOC_GEN_DATA_TYPE_XML || 
				dataType == STLDOC_GEN_DATA_TYPE.STLDOC_GEN_DATA_TYPE_ALL)
			{
				String xmlData = _xmlData == null ?
						StlDoc.getXmlData() : _xmlData;
				_xmlData = setOutputFields(xmlData, tbl);
				StlDoc.setReturnXmlData(_xmlData);
			}
		}
		catch (Exception e) { PluginLog.error("Failed to apply to Xml Data: "+e.toString()); }
