...
import com.jm.accountingfeed.jaxbbindings.salesledgersap.ObjectFactory;
import com.jm.accountingfeed.jaxbbindings.salesledgersap.Trade;
import com.jm.accountingfeed.jaxbbindings.salesledgersap.Trades;
 ...
 * 
 */
public class SalesLedgerSapOutput extends AccountingFeedOutput
{
    /**
	...
	public void cacheXMLData() throws OException
	{
		try
		{
			...
			for (int row = 1; row <= numRows; row++)
			{
				...
				String toCurrency = Ref.getName(SHM_USR_TABLES_ENUM.CURRENCY_TABLE, tblOutputData.getInt("to_currency", row));
				...
				trade.setTradeStatus(Ref.getShortName(SHM_USR_TABLES_ENUM.TRANS_STATUS_TABLE, tranStatus));
				
				String sapCompanyCode = tblOutputData.getString("company_code", row);
				trade.setLglEntity(sapCompanyCode);
				
				String sapBusinessUnit = tblOutputData.getString("business_unit", row);
				trade.setBusinessUnit(sapBusinessUnit);
	...
	public void extractAuditRecords() throws OException
	{
		...
		try
		{
			tableToInsert = Table.tableNew(ExtractionTableName.SALES_LEDGER_SAP.toString());

			int ret = DBUserTable.structure(tableToInsert);
			if (ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt())
			{
				throw new AccountingFeedRuntimeException("Unable to get structure of: " + ExtractionTableName.SALES_LEDGER_SAP.toString());
			}
			...
			for (int row = 1; row <= numRows; row++)
			{
				...
				if (docAlreadyExtracted)
				{
					int existingDocStatus = tableToInsert.getInt(BoundaryTableSalesLedgerSapDataColumns.ENDUR_DOC_STATUS.toString(), rowToSet);
					/* 
					...
					tableToInsert.setInt(BoundaryTableSalesLedgerSapDataColumns.ENDUR_DOC_STATUS.toString(), rowToSet, newDocStatus);
				}
				else
				{
					tableToInsert.setInt(BoundaryTableSalesLedgerSapDataColumns.ENDUR_DOC_NUM.toString(), rowToSet, endurDocNum);
					tableToInsert.setInt(BoundaryTableSalesLedgerSapDataColumns.ENDUR_DOC_STATUS.toString(), rowToSet, endurDocStatus);
					tableToInsert.setClob(BoundaryTableSalesLedgerSapDataColumns.PAYLOAD.toString(), rowToSet, payLoad);	
				}
			}
			...
			if (tableToInsert.getNumRows() > 0)
			{
				...
				if (ret != OLF_RETURN_CODE.OLF_RETURN_SUCCEED.toInt())
				{
					throw new AccountingFeedRuntimeException("Unable to insert record into: " + ExtractionTableName.SALES_LEDGER_SAP.toString());
				}				
			}
		}
