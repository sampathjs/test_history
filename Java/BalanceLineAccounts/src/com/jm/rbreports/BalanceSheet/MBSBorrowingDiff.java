...
public class MBSBorrowingDiff implements IScript{
	...
	{		
		...
		Table outputTable = new Table("outputTable");
		runReportAndFilterBorrowingData(MBS_HK_RPT_NAME, outputTable, "L008 HK-UK Borrowings", "L009 HK-US Borrowing");
		runReportAndFilterBorrowingData(MBS_UK_RPT_NAME, outputTable, "L008 UK-HK Borrowings", "L009 UK-US Borrowings");
		runReportAndFilterBorrowingData(MBS_US_RPT_NAME, outputTable, "L8 US-UK Borrowings", "L9 US-HK Borrowings");

		sumUpColumns(outputTable, returnt);
		outputTable.destroy();
	}

	private void sumUpColumns(Table outputTable, Table returnt) throws OException{
		Table rptTable = new Table("rptTable");
...
//			rptTable.viewTable();
	...
	private void runReportAndFilterBorrowingData(String rptName, Table outputTable, String balanceLine1, String balanceLine2) throws OException{
		Table rptTable = runReport(rptName, OCalendar.getLgbd(OCalendar.today()));
