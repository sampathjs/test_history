...
public class JM_MatureTradesByQid implements IScript {
	...
	public void execute(IContainerContext context) throws OException
	{
		...
		Table tradesToMature = null;
			...
			{
				intRetVal = Transaction.mature(tradesToMature.getInt( "tran_num", loop), tradesToMature.getInt( "version_number", loop));
				if(intRetVal == 1) {
					tradesToMature.copyRowAdd( loop, tOutput);
					PluginLog.info("Matured trade tran num: " + tradesToMature.getInt( "tran_num", loop) );
				}
...
//		}
	...
	private Table loadTrades() throws ConstantTypeException, ConstantNameException, OException {
		String queryName = "MatureMigratedTrades";
		
		queryName = constRep.getStringValue("MatureTradesQuery", queryName);
		
		if(queryName == null || queryName.length() == 0) {
			String errorMessage = "Invalid query name, check the const repository MatureMigratedTradesQuery entry";
		...
		try {
			PluginLog.debug("About to run sql " + what + " " + from + " " + where);
			DBaseTable.execISql(resultTable, what + from + where);
		} catch (Exception e) {
