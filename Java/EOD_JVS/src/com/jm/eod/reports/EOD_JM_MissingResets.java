...
public class EOD_JM_MissingResets implements IScript {
	...
	public void execute(IContainerContext context) throws OException {
		...
		if (symbolicDate != null && symbolicDate.trim().length() > 0) {
			try {
				...
				String message = "Could not parse symbolic date '"
						+ symbolicDate + "' as "
						+ " specified in Constants Repository " + CONTEXT
						+ "\\" + SUBCONTEXT + "\\earliestDate";
		...
		try {
			...
			String dbDate = Utils.getParam(params, Const.QUERY_DATE); // get
																		// date
																		// from
																		// params
			// String dbDate =
			// OCalendar.formatJdForDbAccess(OCalendar.today()-1);
			...
			rptData = createReport(output,
					Utils.getParam(params, Const.REGION_COL_NAME).trim(), Utils
							.getParam(params, Const.QUERY_REPORT).trim());
			// 1.2 to save Global EOD report in CSV format
			String RegionCode = params.getString(
					(Const.REGION_COL_NAME).trim(), 1);
			...
			if (Table.isTableValid(rptData) == 1 && rptData.getNumRows() > 0) {
				PluginLog
						.error("Found deals with missed resets - please check EOD report.");
				Util.scriptPostStatus(output.getNumRows()
						+ " missing reset(s)."); // 1.2 changed table to output
													// from rptdata to correct
													// the num of missing resets
	 ...
	 */
	private Table getDeals(int qid, String dbDate, int jdEarliestDate)
			throws OException {
		Table deals = Util.NULL_TABLE, dates = Util.NULL_TABLE, after;
		try {
			...
			String sql = "SELECT ab.internal_bunit,\n"
					...
					+ ((jdEarliestDate == -1) ? ""
							: ("AND    r.reset_date >= '"
									+ OCalendar
											.formatJdForDbAccess(jdEarliestDate) + "'"))
	 ...
	 * @param dbDate - today's date formated for db access
	 */
	private void getNonCommodityCflowResets(Table output, int qid, String dbDate)
			throws OException {
		...
		try {
			...
			PluginLog.debug("Non Commodity Cashflow Resets: Found "
					+ cflows.getNumRows() + " rows.");
	...
	/*
	 * If deal projection_index is not defined, retrieve index from underlying
	 * instrument... CFLOW_TYPE.PREPAYMENT_PRINCIPAL_CFLOW - spot_index of the
	 * underlying instrument CFLOW_TYPE.COUPON_PAYMENT_CFLOW - proj_index of the
	 * underlying instrument
	 * 
	 * @param cflows: cash flow data
	 */
	private void getUnderlyingIdx(Table cflows) throws OException {
		Table insNums = Util.NULL_TABLE, underlyingIdx = Util.NULL_TABLE, tmp = Util.NULL_TABLE;
		int insNumQid = 0;
		...
		try {
			...
			"SELECT lnk.ins_num deriv_ins_num, lnk.underlying_tran, h.ins_num, p.proj_index, p.spot_index\n"
					...
					+ sqlWhere + "AND    lnk.underlying_tran_num = h.tran_num";
			// + "\n)\n"
			...
			tmp = Table.tableNew();
			tmp.select(cflows, "*", "proj_index EQ 0 and cflow_type EQ "
					+ CFLOW_TYPE.PREPAYMENT_PRINCIPAL_CFLOW.toInt());
			if (tmp.getNumRows() > 0) {
				tmp.select(underlyingIdx, "spot_index(proj_index)",
						"deriv_ins_num EQ $ins_num");
				cflows.select(tmp, "*",
						"tran_num EQ $tran_num and reset_date EQ $reset_date"
								+ " and cflow_type EQ "
								+ CFLOW_TYPE.PREPAYMENT_PRINCIPAL_CFLOW.toInt());
			}
			...
			tmp = Table.tableNew();
			tmp.select(cflows, "*", "proj_index EQ 0 and cflow_type EQ "
					+ CFLOW_TYPE.COUPON_PAYMENT_CFLOW.toInt());
			if (tmp.getNumRows() > 0) {
				tmp.select(underlyingIdx, "proj_index",
						"deriv_ins_num EQ $ins_num");
				cflows.select(tmp, "*",
						"tran_num EQ $tran_num and reset_date EQ $reset_date"
								+ " and cflow_type EQ "
	 ...
	 * @param dbDate - today's date formated for db access
	 */
	private void getCommodityCflowResets(Table output, int qid, String dbDate)
			throws OException {
		Table cflows = Util.NULL_TABLE;
		try {
			String sql = "SELECT ab.internal_bunit,\n"
			...
			cflows = Utils.runSql(sql);
			PluginLog.debug("Commodity Cashflow Resets: Found "
					+ cflows.getNumRows() + " rows.");
		} finally {
			Utils.removeTable(cflows);
		}
	 ...
	 * @param dbDate - today's date formated for db access
	 */
	private void getNotionalResets(Table output, int qid, String dbDate)
			throws OException {
		...
		try {
			...
			String sql = "SELECT ab.internal_bunit,\n"
					...
					+ dbDate
					+ "'\n"
					+ "AND    rst.reset_value_status = "
					+ VALUE_STATUS_ENUM.VALUE_UNKNOWN.toInt()
					+ "\n"
					+ "AND    rpct.rst_param_calc_type_id = rst.reset_calc_type";

			notnl = Utils.runSql(sql);
			PluginLog.debug("Notional Resets: Found " + notnl.getNumRows()
					+ " rows.");
	 ...
	 * @param qryName: name of query to execute
	 */
	private void getNotionalCcyResets(Table output, int qid, String dbDate)
			throws OException {
		Table notnl = Util.NULL_TABLE;
		try {
			String sql = "SELECT ab.internal_bunit,\n"
					...
					+ "AND    pnr.reset_status = "
					+ VALUE_STATUS_ENUM.VALUE_UNKNOWN.toInt() + "\n"
					+ "AND    pcp.ins_num = p.ins_num\n"
					+ "AND    pcp.param_seq_num = p.param_seq_num";

			notnl = Utils.runSql(sql);
			PluginLog.debug("Notional Currency Resets: Found "
					+ notnl.getNumRows() + " rows.");
	 ...
	 * @param
	 */
	private Table createReport(Table data, String regionCode, String filename)
			throws OException {
		...
		String title = "Missed Resets Report for "
				+ OCalendar.formatJd(OCalendar.today(),
	...
	private void formatOutput(Table report, int report_type) throws OException {
		...
		report.setColFormatAsRef("internal_bunit",
				SHM_USR_TABLES_ENUM.PARTY_TABLE);
		report.setColFormatAsRef("tran_status",
				SHM_USR_TABLES_ENUM.TRANS_STATUS_TABLE);
		report.setColFormatAsRef("ins_type",
				SHM_USR_TABLES_ENUM.INSTRUMENTS_TABLE);
		report.setColFormatAsRef("proj_index", SHM_USR_TABLES_ENUM.INDEX_TABLE);
		report.setColFormatAsRef("index_src",
				SHM_USR_TABLES_ENUM.REF_SOURCE_TABLE);
		report.setColFormatAsRef("internal_portfolio",
				SHM_USR_TABLES_ENUM.PORTFOLIO_TABLE);
