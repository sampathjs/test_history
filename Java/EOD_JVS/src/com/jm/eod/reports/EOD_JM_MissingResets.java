...
//				Table after;
		try {
			...
			String sql = "SELECT ab.internal_bunit,\n"
					...
					+ ((jdEarliestDate == -1) ? "" : ("AND    r.reset_date >= '" + OCalendar.formatJdForDbAccess(jdEarliestDate) + "'"))
	...
	private void getUnderlyingIdx(Table cflows) throws OException {
		...
		try {
			...
			if (tmp.getNumRows() > 0) {
				...
				cflows.select(tmp, "*", "tran_num EQ $tran_num and reset_date EQ $reset_date" + " and cflow_type EQ " + CFLOW_TYPE.PREPAYMENT_PRINCIPAL_CFLOW.toInt());
			}
			...
			if (tmp.getNumRows() > 0) {
				...
				cflows.select(tmp, "*", "tran_num EQ $tran_num and reset_date EQ $reset_date" + " and cflow_type EQ " + CFLOW_TYPE.COUPON_PAYMENT_CFLOW.toInt());
			}
		} finally {
			if (insNumQid > 0) {
	...
	private Table createReport(Table data, String regionCode, String filename) throws OException {
		...
		String title = "Missed Resets Report for " + OCalendar.formatJd(OCalendar.today(), DATE_FORMAT.DATE_FORMAT_DMLY_NOSLASH);
