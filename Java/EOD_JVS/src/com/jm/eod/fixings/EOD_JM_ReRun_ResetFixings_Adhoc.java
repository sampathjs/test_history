...
public class EOD_JM_ReRun_ResetFixings_Adhoc implements IScript
{	
	...
	public EOD_JM_ReRun_ResetFixings_Adhoc() {
    ...
    {       
        ...
        try{
        	...
        	String queryName = params.getString("queryName", 1);
        	...
        	for(int i=1;i<=intNumDays;i++){
        		...
        		Logging.debug("Running query "+queryName+" for current date  "  + OCalendar.formatDateInt(intCurrDate));
        		
            	int query_id =  Query.run(queryName);
            	Logging.debug("Running query id = "+ query_id );
                
            	String strSQL = "SELECT * from " + Query.getResultTableForId(query_id) + "  where unique_id = " + query_id;
            	Logging.debug("Running strSQL = " + strSQL);
