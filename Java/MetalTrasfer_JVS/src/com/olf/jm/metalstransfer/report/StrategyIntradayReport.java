...
public class StrategyIntradayReport  implements IScript {
	...
	private Table fetchTaxData() throws OException {
		...
		PluginLog.info("Number of records returned for processing: " + taxMismatch.getNumRows());
	...
	private Table fetchReportdata() throws OException {
	...
	private Table fetchTPMfailure() throws OException {
		...
		PluginLog.info("Number of records returned for processing: " + failureData.getNumRows());
	} catch (Exception exp) {
		PluginLog.error("Error while fetching startegy Deals " + exp.getMessage());
		throw new OException(exp);
	}
