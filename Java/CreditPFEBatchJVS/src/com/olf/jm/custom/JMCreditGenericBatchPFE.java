...
public class JMCreditGenericBatchPFE implements IScript 
{
   ...
   public JMCreditGenericBatchPFE()
   {
   ...
   public void execute(IContainerContext context) throws OException
   {
      ...
      String queryName = "CreditPFEDeals";
      ConstRepository constRepo = new ConstRepository(CONST_REPO_CONTEXT, CONST_REPO_SUBCONTEXT);
      queryName = constRepo.getStringValue("queryName", queryName);
      where = " ab_tran.trade_flag = 1 AND ab_tran.tran_num IN ( SELECT query_result tran_num FROM query_result WHERE unique_id  = "
    		  + Query.run(queryName) + ")";
