...
-- "Cancelled" status, where the sap_status is in a "Pending" state.
...
AND shh.stldoc_template_id IN (select stldoc_template_id from stldoc_templates where stldoc_template_name like 'JM-Invoice%') -- 'JM Invoice' templates 
...
AND udt.sap_status IN ('Pending Sent', 'NOT Sent')