...
SELECT account_id, 
       ...
       CASE 
         WHEN unit_id IS NULL 
               OR unit_id = 0 THEN 51 
         ELSE unit_id 
       END                  AS unit 
FROM   (SELECT ates.ext_account_id   AS account_id, 
               ates.currency_id, 
               ates.delivery_type, 
               ab.internal_portfolio AS portfolio_id, 
               -ates.settle_amount   AS pos 
               --, (case when ab.ins_type = 27001 then -ates.settle_amount / 31.103477 else -ates.settle_amount  END) as pos 
               , 
               iu.unit_id 
        FROM   ab_tran_event_settle ates 
               ...
               LEFT JOIN account_info acci 
                      ON acc.account_id = acci.account_id 
                         AND acci.info_type_id = 20003 
               LEFT JOIN idx_unit iu 
