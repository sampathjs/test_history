...
SELECT ubf.target_index_id AS index_id, ISNULL(ihp.ref_source, 0) AS ref_source, ccy.id_number as metal, idx.currency, AVG(ihp.price) AS monthly_price
   ...
   LEFT OUTER JOIN (
		SELECT  DATEADD(DAY, nbr - 1, '$$reset_date_start$$') AS reset_date
        FROM(
			SELECT ROW_NUMBER() OVER ( ORDER BY c.object_id ) AS Nbr -- Logic to get a sequence of numbers starting with 1 to be used in the day-date
