...
SELECT account_id,currency_id, delivery_type, portfolio_id, SUM(toz_position) ohd_position, '$$Reporting_Date$$' as report_date, 51 as unit
FROM (SELECT ates.int_account_id as account_id , nadv.currency_id, nadv.delivery_type, nadv.portfolio_id ,nadv.ohd_position toz_position, 
		(CASE ab.ins_sub_type WHEN 10001  THEN 51 ELSE nadv.unit END) unit, '$$Reporting_Date$$' as report_date
	  ...
	  WHERE nadv.event_date<='$$Reporting_Date$$'
		AND nadv.event_num IN (  
			...
			JOIN ab_tran ab ON (ate.tran_num=ab.tran_num  AND ab.current_flag=1 AND ab.tran_status IN (SELECT trans_status_id FROM trans_status WHERE name in ('Validated', 'Matured', 'Closeout')))   
