--tabletitle D960_NostroTL "Nostro Transaction Listing"
...
SELECT  a.account_number
     , ates.int_account_id as NostroAccount_id 
    , a.account_type
    , a.account_class
...
FROM ab_tran ab  
    ...
    INNER JOIN account a ON a.account_id=ates.int_account_id AND a.account_number = '$$accountNumber$$'
...
WHERE  ab.tran_status IN (
		SELECT trans_status_id FROM trans_status 
		WHERE name in ('Validated', 'Matured', 'Closeout')) 
	AND ab.current_flag=1 
	AND (ab.offset_tran_num<1 OR ab.offset_tran_type IN (2, 5)) 
	$$tradeRange$$