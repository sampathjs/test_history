...
IF OBJECT_ID('USER_archive_per_audit_table', 'P') IS NOT NULL
       DROP PROC USER_archive_per_audit_table

GO
CREATE procedure [dbo].[USER_archive_per_audit_table]
          @table_name SYSNAME,
...
BEGIN
       ...
       SET @criteria = 'FROM '+ @table_name +' WHERE '+@column_name + ' < ''' +CONVERT(VARCHAR(20),@retain_date)+ ''' AND latest_version = 0';    
		...
		SET @criteria = 'FROM '+ @table_name +' WHERE '+@column_name + ' < ''' +CONVERT(VARCHAR(20),@retain_date)+ ''' AND per_pesonnel_status = ''Do not use''  ';    
...
GRANT EXEC ON USER_archive_per_audit_table TO olf_user;
GRANT EXEC ON USER_archive_per_audit_table TO olf_user_manual;