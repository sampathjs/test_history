...
public class ReferenceOrderRepoTest extends AbstractRepositoryTestBase<ReferenceOrder, OrderVersionId, ReferenceOrderRepository> {
	@Autowired
	...
	@Autowired
	protected ReferenceOrderLegConverter legConverter;
	...
	@Override
	protected Supplier<List<ReferenceOrder>> listProvider() {
		// TODO: Add Limit Order with all optional fields being null
		return () -> {
			// 1st test case with all fields filled
			List<ReferenceOrder> list =  Arrays.asList(new ReferenceOrder (referenceConverter.toManagedEntity(DefaultReference.ORDER_TYPE_LIMIT_ORDER.getEntity()), // order type name
										1,   // version
										...
										// << order fields
										100d,  // metalPriceSpread
										200d, // fxRateSpread
										300d, // contangoBackwardation
										Arrays.asList(legConverter.toManagedEntity(TestReferenceOrderLeg.TEST_LEG_1.getEntity()),
												legConverter.toManagedEntity(TestReferenceOrderLeg.TEST_LEG_2.getEntity())) // legs
										)
					// 2nd test case with only mandatory fields being filled.
					,new ReferenceOrder (referenceConverter.toManagedEntity(DefaultReference.ORDER_TYPE_LIMIT_ORDER.getEntity()), // order type name
							1,   // version
							partyConverter.toManagedEntity(TestBunit.JM_PMM_UK.getEntity()),  // internal bu 
							partyConverter.toManagedEntity(TestBunit.ANGLO_PLATINUM_MARKETING___BU.getEntity()), // external bu
							partyConverter.toManagedEntity(TestLenit.JM_PLC.getEntity()),  // internal le
							...
							userConverter.toManagedEntity(TestUser.ANDREW_BAYNES.getEntity()),  // updated by
							0.0d, // fillPercentage, going to get updated automatically before persisting to database
							referenceConverter.toManagedEntity(DefaultReference.CONTRACT_TYPE_LIMIT_FIXED.getEntity()), // contract type
							referenceConverter.toManagedEntity(DefaultReference.TICKER_XAUEUR.getEntity()), // ticket
							Arrays.asList(orderCommentConverter.toManagedEntity(TestOrderComment.TEST_COMMENT_2.getEntity())), // order comments
							Arrays.asList(fillConverter.toManagedEntity(TestFill.TEST_LIMIT_ORDER_FILL_2.getEntity())),  // fills
							Arrays.asList(creditCheckConverter.toManagedEntity(TestCreditCheck.TEST_CREDIT_CHECK_1.getEntity())), // credit checks
							// << order fields
							100d,  // metalPriceSpread
							200d, // fxRateSpread
							300d, // contangoBackwardation
							Arrays.asList(legConverter.toManagedEntity(TestReferenceOrderLeg.TEST_LEG_4.getEntity()),
									legConverter.toManagedEntity(TestReferenceOrderLeg.TEST_LEG_5.getEntity()))) // legs
					);
	...
	@Override
	protected Function<ReferenceOrder, OrderVersionId> idProvider() {
