...
USE [DBA]
GO
/****** Object:  StoredProcedure [AppSupport].[TPM_Working]    Script Date: 25/07/2018 09:58:37 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROC [AppSupport].[TPM_Working] (@debug TINYINT = 0, @email_address VARCHAR(1000) ='charles.badcock@matthey.com')
-------------------------------------------------------
-- APPSupport.TPM_Working
...
-- Email:  Support Team
-- Frequency: Every 10 minutes throughout the from 12.15 am to 11.30 pm
...
BEGIN
	...
	IF OBJECT_ID('tempdb..##TPM_Working') IS NOT NULL DROP TABLE ##TPM_Working
	...
	SET @sql_stmt = 'SELECT @is_working =  CASE WHEN ISNULL(jr1.status,0) = 18 THEN 1 ELSE (CASE WHEN ISNULL(jr2.status,0) = 18 THEN 1 ELSE 0 END) END as working 					
					...
					WHERE jc_h.name = ''Trade Process Mgmt'''
	...
	IF @is_working = 0 BEGIN
		...
		SET @email_query = 'SELECT * from ##TPM_Working'
